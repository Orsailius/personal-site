<!-- Based on https://svelte.dev/repl/75d34e46cbe64bb68b7c2ac2c61931ce?version=4.2.9 -->
<script lang='ts'>
	...
	function setHandlePosition (which: string) {
		return function (evt: { detail: { x: number; }; }) {
			...
			if (which === 'start') {
				start.set(p);
				end.set(Math.max($end, p));
			} else {
				start.set(Math.min(p, $start));
				end.set(p);
			}
		}
	}
...
<div class="double-range-container p-2">
