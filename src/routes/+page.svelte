<script lang="ts">
    import ComponentPreviewBox from '$lib/components/ComponentPreview.svelte';
	import SkillList from '$lib/components/SkillList.svelte';
	import { programmingLanguages, tools } from '$lib/SkillData';
	import { projects } from '$lib/datatypes/ProjectInfo';
    import ProjectCard from '$lib/components/ProjectCard.svelte';
</script>

<svelte:head>
	<title>Home</title>
	<meta name="description" content="Justin Moore" />
</svelte:head>

<!-- svelte-ignore empty-block -->
{#if true}
	<div class="bg-base-100">
		<section class="hero min-h-screen">
			<div class="hero-content text-center">
				<div class="max-w-md">
					<h1 class="text-5xl font-bold">Coming Soon</h1>
					<p class="py-6">Stay tuned for the website, you can check out some other content in the tabs at the top of the screen</p>
					<p class="py-6">Thanks, Justin</p>
					<a href="/blanket"> Blanket Designer </a>
				</div>
			</div>  
		</section>
		<!--<section>
			<h1 class="text-4xl pb-8 font-bold"> Projects </h1>
			<div class="grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-4">
				{#each projects as projectInfo}
					<ProjectCard name={projectInfo.name} 
						url="src/lib/images/test/FicterraCard.png" 
						description={projectInfo.shortDescription} />
				{/each}
			</div>		
		</section>-->
	</div>
	
{:else}
	<section class="bg-slate-100 m:p-[16vw] p-[12vw]">	
		<div class="grid grid-cols-2">
			<div id="shortDescription">
				<h2 class="font-title text-[clamp(2.5rem,6vw,4.5rem)] leading-none col-span-1">
					Justin Moore
				</h2>
				<h2 class="font-title text-[clamp(1.5rem,3vw,2.5rem)] leading-1 col-span-1 text-blue-700">
					Game Developer
				</h2>
				<p class="text-base-content/60 font-title py-4 font-light md:text-2xl"> Lorem ipsum stuff and things over he moon and other things who is ie?</p>
			</div>
			
			<!-- svelte-ignore a11y-missing-attribute -->
			<img src="src/lib/images/github.svg" />
		</div>		
	</section>
	
	<section class="bg-blue-500 relative lg:h-[90vh] md:h-[120vh] h-[180vh]">
		<div class="absolute top-16">
			<div class="mb-16"></div>
			<SkillList skills={programmingLanguages} title="Languages"/>
			<div class="mb-16"></div>
			<SkillList skills={tools} title="Tools"/>
		</div>
	
		<!--<div class="top-0">
			Things
		</div>
		<div class="sticky top-[16vh] xl:top-[30h] text-white">
			Stuff
		</div>-->
		<div class="introDivider">
			<svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
				<path d="M0,0V46.29c47.79,22.2,103.59,32.17,158,28,70.36-5.37,136.33-33.31,206.8-37.5C438.64,32.43,512.34,53.67,583,72.05c69.27,18,138.3,24.88,209.4,13.08,36.15-6,69.85-17.84,104.45-29.34C989.49,25,1113-14.29,1200,52.47V0Z" opacity=".25" class="shape-fill"></path>
				<path d="M0,0V15.81C13,36.92,27.64,56.86,47.69,72.05,99.41,111.27,165,111,224.58,91.58c31.15-10.15,60.09-26.07,89.67-39.8,40.92-19,84.73-46,130.83-49.67,36.26-2.85,70.9,9.42,98.6,31.56,31.77,25.39,62.32,62,103.63,73,40.44,10.79,81.35-6.69,119.13-24.28s75.16-39,116.92-43.05c59.73-5.85,113.28,22.88,168.9,38.84,30.2,8.66,59,6.17,87.09-7.5,22.43-10.89,48-26.93,60.65-49.24V0Z" opacity=".5" class="shape-fill"></path>
				<path d="M0,0V5.63C149.93,59,314.09,71.32,475.83,42.57c43-7.64,84.23-20.12,127.61-26.46,59-8.63,112.48,12.24,165.56,35.4C827.93,77.22,886,95.24,951.2,90c86.53-7,172.46-45.71,248.8-84.81V0Z" class="shape-fill"></path>
			</svg>
		</div>
	</section>
	
	<section class="bg-lime-500">
		<div class="secondDivider">
			<svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
				<path d="M321.39,56.44c58-10.79,114.16-30.13,172-41.86,82.39-16.72,168.19-17.73,250.45-.39C823.78,31,906.67,72,985.66,92.83c70.05,18.48,146.53,26.09,214.34,3V0H0V27.35A600.21,600.21,0,0,0,321.39,56.44Z" class="shape-fill"></path>
			</svg>
		</div>
	</section>
	
	<section class="bg-purple-500">
		<div class="thirdDivider">
			<svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
				<path d="M1200 120L0 16.48 0 0 1200 0 1200 120z" class="shape-fill"></path>
			</svg>
		</div>		
	</section>	
{/if}

<style lang="postcss">
	section {
		display: flex;
		position: relative;
		flex-direction: column;
		justify-content: center;
		align-items: center;
		min-height: 400px;
		flex: 0.6;
	}

	.center {
		display: flex;
		flex-direction: column;
		justify-content: center;
		align-items: center;
		flex: 0.6;
	}

	h1 {
		width: 100%;
	}

	.welcome {
		display: block;
		position: relative;
		width: 100%;
		height: 0;
		padding: 0 0 calc(100% * 495 / 2048) 0;
	}


.introDivider {
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
    overflow: hidden;
    line-height: 0;
}

.introDivider svg {
    position: relative;
    display: block;
    width: calc(125% + 1.3px);
    height: 126px;
}

.introDivider .shape-fill {
    fill: theme(colors.slate.100);
}

.secondDivider{
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
    overflow: hidden;
    line-height: 0;
}

.secondDivider svg {
    position: relative;
    display: block;
    width: calc(125% + 1.3px);
    height: 126px;
	transform: rotateY(180deg);
}

.secondDivider .shape-fill {
    fill: theme(colors.blue.500);
}

.thirdDivider {
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
    overflow: hidden;
    line-height: 0;
}

.thirdDivider svg {
    position: relative;
    display: block;
    width: calc(125% + 1.3px);
    height: 126px;
    transform: rotateY(180deg);
}

.thirdDivider .shape-fill {
    fill: theme(colors.lime.500);
}


	/*.welcome img {
		position: absolute;
		width: 100%;
		height: 100%;
		top: 0;
		display: block;
	}*/
</style>
